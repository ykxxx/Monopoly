...
Monopoly::GameState::GameState(const std::string& RulesFileName,
  ...
  for (auto& player : players) {
    player->setOn(board.getSpaceRef(0), false);
  }
...
void Monopoly::GameState::make_move(Monopoly::Player& player, Monopoly::Move& move) {
  ...
  } else if (move.getAction() == MoveAction::rollDice) {
    rollDice(player);
...
void Monopoly::GameState::rollDice(Monopoly::Player& player) {
  ...
  int salary = static_cast<Monopoly::Go&> (*board.getGoSpace()).getSalary();
  int amount = 0;

  //this->board.getSpace(newLocation);
  if (timesPastGo > 1) {
    amount = (timesPastGo - 1) * salary + salary * rules.getSalary_multiplier_for_landing_on_go();
  }
  else if (timesPastGo == 1) {
    amount = timesPastGo * salary;
  }
  ...
  if (player.inJail) {
    ...
    //newSpace = this->board.getSpace(newLocation);

    player.moveTo(this->board.getSpaceRef(newLocation));
