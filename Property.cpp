...
Monopoly::Property::Property(CSVReader& boardFile, GameState& gameState, const Rules& rules, const int spaceNumber)
    : Space(boardFile, gameState, spaceNumber), owner(nullptr), numHousesOn(0), numHotelsOn(0), rules(rules) {
...
void Monopoly::Property::activate(Monopoly::Player &activatingPlayer) {
  ...
  if (owner == nullptr) {
    if (activatingPlayer.getCash() >= getCost()) { //can afford the property
      ...
      } else if(rules.isAuction_properties()){
    ...
    activatingPlayer.payPlayer(*owner, *this, rules);
  }
